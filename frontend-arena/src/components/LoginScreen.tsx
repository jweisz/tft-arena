...
export const LoginScreen: React.FC<{
  ...
  const handleLocalLogin = async () => {
    ...
    try {
      const payload = await apiJson<VerifyGoogleTokenResponse>(
      ...
      setError(
        getErrorMessage(
          err,
          "Could not reach backend auth endpoint; using local mode.",
        ),
      );
  ...
  return (
    ...
    >
      <div style={{ textAlign: "center", marginBottom: "2rem" }}>
        <h1 style={{ margin: 0, fontSize: "2.5rem", letterSpacing: "0.05em" }}>
          TFT ARENA
        </h1>
        <p style={{ color: "var(--text-secondary)", marginTop: "0.5rem" }}>
          Can your ideas survive the onslaught?
        </p>
      </div>
