...
export const SidebarRight: React.FC<Props> = ({
  roomId,
  scratchpad,
  semanticLastUpdatedAt,
  telemetry,
}) => {
  const { agentsRefreshKey, agentStatuses, agentBudgets } = useUIStore();
  const hasContent =
    scratchpad &&
    (scratchpad.consensus ||
      scratchpad.open_questions.length > 0 ||
      scratchpad.key_ideas.length > 0);
  ...
  const toggleAgent = async (agentId: number) => {
    ...
    try {
      const res = await apiFetch(
        `/api/rooms/${roomId}/agents/${agentId}/toggle`,
        {
          method: "POST",
        },
      );
  ...
  const bulkActiveAgents = async (active: boolean) => {
    ...
    try {
      const res = await apiFetch(
        `/api/rooms/${roomId}/agents/bulk-active?active=${active}`,
        {
          method: "POST",
        },
      );
  ...
  const getSemanticStatus = () => {
    ...
    const ageSeconds = Math.max(
      0,
      Math.floor((nowMs - semanticLastUpdatedAt) / 1000),
    );
    if (ageSeconds <= 15) {
      return {
        label: "Fresh",
    ...
    <aside className="agents-sidebar">
      ...
      >
        ...
        >
          ...
          {agents.map((agent) => {
            const currentBudget =
              agentBudgets[agent.name] ?? agent.token_budget;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: isActive
                    ? `1px solid ${isProcessing ? getStatusColor(status) : "var(--border-color)"}`
                  ...
                  boxShadow: isProcessing
                    ? `0 0 10px ${getStatusColor(status)}22`
                    : "none",
                }}
              >
                <div
      ...
      >
        ...
        >
          {hasContent ? (
              ...
              {scratchpad?.key_ideas && scratchpad.key_ideas.length > 0 && (
                <div style={{ marginBottom: "0.75rem" }}>
                  ...
                  <ul style={{ margin: 0, paddingLeft: "1.25rem" }}>
                    {scratchpad.key_ideas.map((idea, i) => (
                      <li key={i}>{idea}</li>
                    ))}
                  </ul>
                </div>
              ...
              {scratchpad?.open_questions &&
                scratchpad.open_questions.length > 0 && (
                  <div>
                    ...
                    <ul style={{ margin: 0, paddingLeft: "1.25rem" }}>
                      {scratchpad.open_questions.map((q, i) => (
                        <li key={i}>{q}</li>
                      ))}
                    </ul>
                  </div>
            ...
            >
              Start a discussion — the Semantic Agent will continuously update
              this whiteboard.
            </p>
          )}
        </div>
