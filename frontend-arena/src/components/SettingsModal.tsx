...
export const SettingsModal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"theme" | "models" | "arena">(
    "theme",
  );
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      ...
      try {
        const data =
          await apiJson<Record<string, string | number | null>>(
            "/api/settings/",
          );
        if (!cancelled) {
  ...
  const selectedNonAgent = (() => {
    const hasSelection = availableModels.some(
      (provider) =>
        provider.provider === nonAgentProvider &&
        provider.models.includes(nonAgentModel),
    );
    if (hasSelection) {
                      ...
                      style={{
                        ...
                        border:
                          palette === p.id
                            ...
                            : `2px solid ${p.border}`,
                        boxShadow:
                          palette === p.id ? `0 0 15px ${p.color2}40` : "none",
                        transition: "all 0.2s ease",
                      }}
                    />
                    <span
                      style={{
                        fontSize: "0.75rem",
                        fontWeight: palette === p.id ? "bold" : "normal",
                        textAlign: "center",
                      }}
                    >
                      {p.name}
            ...
            >
              <div>
                ...
                {(hasOpenAIKey || clearOpenAIKey) && !openaiKey.trim() && (
                  ...
                  >
                    {clearOpenAIKey
                      ? "OpenAI key will be removed on save"
              ...
              <div>
                ...
                {(hasAnthropicKey || clearAnthropicKey) &&
                  !anthropicKey.trim() && (
                    ...
                    >
                      {clearAnthropicKey
                        ? "Anthropic key will be removed on save"
                        : "Remove saved Anthropic key"}
                    </button>
                  )}
              </div>
              <div>
                ...
                {(hasGeminiKey || clearGeminiKey) && !geminiKey.trim() && (
                  ...
                  >
                    {clearGeminiKey
                      ? "Gemini key will be removed on save"
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {ollamaStatus === "connected" && (
                      ...
                      >
                        <CheckCircle size={12} /> {ollamaModelCount}{" "}
                        {ollamaModelCount === 1 ? "model" : "models"}
                      </span>
                    )}
                    {ollamaStatus === "error" && (
                      <span
                ...
                >
                  Use host.docker.internal if running Ollama natively on
                  Mac/Windows.
                </span>
                ...
                {ollamaStatus === "error" && (
                  ...
                  >
                    ...
                    >
                      Could not connect to Ollama. Make sure it's running and{" "}
                      <code>OLLAMA_HOST</code> is set to <code>0.0.0.0</code>.
                    </p>
                  </div>
                )}
              </div>
        ...
        {activeTab === "arena" && (
          <div style={{ animation: "fadeSlideIn 0.2s ease" }}>
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              >
                Non-Agent Inference Model
              </label>
              {availableModels.length === 0 ? (
                <div
                  style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}
                >
                  No models discovered yet. Configure provider keys or connect
                  Ollama.
                </div>
              ) : (
                <select
                  value={encodeModelSelection(
                    selectedNonAgent.provider,
                    selectedNonAgent.model,
                  )}
                  onChange={(e) => {
              ...
              >
                Used by hidden arena inference: router scoring, conversation
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <input
                ...
                value={defaultBudget}
                onChange={(e) =>
                  setDefaultBudget(parseInt(e.target.value) || 1)
                }
                style={{
                  width: "100%",
                  padding: "0.75rem",
              ...
              >
                This is the initial number of turns each agent receives in a new
                session. Agents use 1 turn every time they speak.
              </span>
            </div>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              >
                This prompt is prepended to the system prompt of every agent.
                Use it for overall style constraints (e.g. "Speak like an IRC
                user", "Use only 1-2 sentences", etc).
              </span>
            </div>
          </div>
        )}
