...
async def send_semantic_update(
    ...
    if inference_processes is not None:
        set_process_runtime(
            inference_processes, "semantic", active=True, tokens_per_sec=None
        )
    ...
    try:
        ...
        if inference_processes is not None:
            set_process_runtime(
                inference_processes, "semantic", active=False, tokens_per_sec=None
            )
            await manager.send_json_to_room(
                {
                    "type": "inference_status",
...
async def schedule_semantic_update(
    ...
    asyncio.create_task(
        send_semantic_update(
            room_id, semantic_messages, inference_processes=inference_processes
        )
    )
