...
describe("AgentManager", () => {
  ...
  it("creates a new agent persona and refreshes the roster", async () => {
    ...
    const triggerAgentsRefresh = vi.spyOn(
      useUIStore.getState(),
        ...
        json: async () => [
          {
            ...
            relevance_instructions: "Respond to planning requests.",
            system_prompt: "stub",
          },
        ],
      } as Response)
  ...
  it("reorders agents via drag and drop and persists the new order", async () => {
    ...
    fireEvent.dragStart(
      analystDragHandle.closest('div[draggable="true"]') as HTMLElement,
    );
  ...
  it("applies a selected model to all agents", async () => {
    ...
    fetchMock
      .mockResolvedValueOnce({
        ok: true,
        json: async () => initialAgents,
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => [
          { provider: "ollama", models: ["llama3", "llama3.1"] },
        ],
      } as Response)
      .mockResolvedValueOnce({ ok: true, json: async () => [] } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ ...initialAgents[0], model: "llama3.1" }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ ...initialAgents[1], model: "llama3.1" }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => [
  ...
  it("creates all default agents from presets that are missing", async () => {
      ...
      .mockResolvedValueOnce({
        ...
        json: async () => [
          ...
          },
          {
            name: "Skeptic",
            emoji: "🧐",
            role_description: "Challenges assumptions.",
            relevance_instructions: "Respond to risky ideas.",
            system_prompt: "skeptic prompt",
          },
        ],
      } as Response)
      .mockResolvedValueOnce({
    ...
    await user.click(screen.getByRole("button", { name: /advanced/i }));
    await user.click(
      screen.getByRole("button", { name: /create all default agents/i }),
    );
  ...
  it("removes all agents from the advanced tab", async () => {
    ...
    fetchMock
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ message: "Agent 1 deleted" }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ message: "Agent 2 deleted" }),
      } as Response)
      .mockResolvedValueOnce({ ok: true, json: async () => [] } as Response);
    ...
    await user.click(screen.getByRole("button", { name: /advanced/i }));
    await user.click(
      screen.getByRole("button", { name: /remove all agents/i }),
    );
  ...
  it("downloads an agent persona markdown spec", async () => {
        ...
        json: async () => [
          {
            ...
            sort_order: 1,
            role_description: "Analyzes.",
            relevance_instructions: "Respond to analysis.",
    ...
    await screen.findByText("Analyst");
    await user.click(
      screen.getByRole("button", { name: /download analyst markdown/i }),
    );
  ...
  it("imports an agent persona markdown file into the editor form", async () => {
    ...
    fetchMock
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => [{ provider: "ollama", models: ["llama3.1"] }],
      } as Response)
      .mockResolvedValueOnce({ ok: true, json: async () => [] } as Response);
...
Surface trade-offs and feedback loops.`;
    ...
    const file = new File([markdown], "systems-thinker.md", {
      type: "text/markdown",
    });
    ...
    expect(
      await screen.findByDisplayValue("Systems Thinker"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByDisplayValue("Surface trade-offs and feedback loops."),
    ).toBeInTheDocument();
