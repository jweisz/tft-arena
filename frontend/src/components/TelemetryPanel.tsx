...
export const TelemetryPanel: React.FC<Props> = ({ inferenceProcesses }: Props) => {
  const [dotCount, setDotCount] = React.useState(0)

  const activeProcesses = inferenceProcesses.filter((process) => process.active)
  const hasWarming = inferenceProcesses.some((p) => p.active && p.tokens_per_sec === null)
  const streamingAgentProcesses = activeProcesses.filter(
    (process) => process.process_kind === 'agent' && (process.tokens_per_sec ?? 0) > 0,
  )
  const streamingProcesses = activeProcesses.filter((process) => (process.tokens_per_sec ?? 0) > 0)
  const warmingAgentProcesses = activeProcesses.filter(
    (process) => process.process_kind === 'agent' && process.tokens_per_sec === null,
  )

  const pickByTpsThenId = (processes: InferenceProcessStatus[]) => [...processes].sort((a, b) => {
  ...
  const displayProcess = pickByTpsThenId(streamingAgentProcesses)
  ...
  const modelRows = Array.from(
    inferenceProcesses.reduce((acc, process) => {
      ...
      const current = acc.get(modelName) ?? { model: modelName, tokensPerSec: 0, warming: false }
    ...
    <div style={{ padding: '0.75rem 1rem', borderTop: '1px solid var(--border-color)', backgroundColor: 'var(--bg-secondary)', marginTop: 'auto' }}>
      ...
      <div style={{ fontSize: '0.78rem', fontFamily: 'monospace', display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ color: 'var(--text-secondary)' }}>Active process:</span>
          <span style={{ color: displayProcess ? 'var(--text-primary)' : 'var(--text-secondary)' }}>{currentProcess}</span>
        </div>

        {modelRows.length > 0 && (
          <div style={{ marginTop: '0.4rem' }}>
            <span style={{ color: 'var(--text-secondary)', display: 'block', marginBottom: '0.3rem', fontSize: '0.7rem' }}>Model throughput:</span>
            {modelRows.map((row) => {
              ...
              return (
                ...
                >
                  <span style={{ color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}>{row.model}</span>
                  <span style={{ color: isActive ? 'var(--accent-color)' : 'var(--text-secondary)', flexShrink: 0, paddingLeft: '0.4rem' }}>
                    {row.tokensPerSec > 0
