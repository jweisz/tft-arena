...
export const SidebarLeft: React.FC<Props> = ({
  ...
  const hasNonIdleStatus = Object.values(agentStatuses).some(
    (status) => status !== "Idle",
  );
  const isGenerating =
    generationInProgress || streamingAgents.size > 0 || hasNonIdleStatus;
  ...
  const deleteRoom = async (id: number, e?: React.MouseEvent) => {
    ...
    try {
      ...
      if (id === selectedRoomId) {
        const nextRoomId =
          updatedRooms && updatedRooms.length > 0 ? updatedRooms[0].id : 0;
        onSelectRoom(nextRoomId);
      }
    } catch (error) {
      console.error("Delete failed:", error);
      alert(
        `Failed to delete chat: ${getErrorMessage(error, "Internal error")}`,
      );
    }
  };
  ...
  const triggerEmergencyStop = async () => {
    ...
    await apiFetch(`/api/rooms/${selectedRoomId}/emergency-stop`, {
      method: "POST",
    }).catch(console.error);
  };
                  ...
                  >
                    <button
                      ...
                      style={{
                        ...
                        color: "var(--text-secondary)",
                        opacity:
                          room.id === selectedRoomId || menuOpenId === room.id
                            ? 1
                            : 0.4,
                      }}
                    >
